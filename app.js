...
(() => {
  ...
  function shell() {
    const nav = [['index', t('Beranda', 'Home'), 'home'], ['shop', t('Koleksi', 'Shop'), 'shop'], ['about', t('Cerita kami', 'About'), 'about'], ['faq', 'FAQ', 'faq'], ['custom', t('Pesanan khusus', 'Custom orders'), 'custom']];
    ...
    document.querySelector('#site-bottom').innerHTML = `<footer><div class="footer-top"><div><a class="wordmark" href="${url('index')}">solpaċi</a><p class="small">${t('Sedikit warna. Banyak cerita.<br>Sebuah ruang kecil untuk menjadi dirimu.', 'A little color. A lot of stories.<br>A little space to be yourself.')}</p></div>
      <nav aria-label="${t('Jelajahi', 'Explore')}"><p class="eyebrow">${t('Jelajahi', 'Explore')}</p><a href="${url('shop')}">${t('Semua warna', 'All shades')}</a><a href="${url('about')}">${t('Cerita Solpaċi', 'Our story')}</a><a href="${url('custom')}">${t('Pesanan khusus', 'Custom orders')}</a><a href="${url('cart')}">${t('Keranjang demo', 'Demo bag')}</a></nav>
    ...
    const titles = { home: t('Beranda', 'Home'), shop: t('Koleksi warna', 'Shop shades'), product: t('Detail shade', 'Shade details'), cart: t('Keranjang', 'Your bag'), checkout: t('Checkout simulasi', 'Demo checkout'), about: t('Cerita kami', 'Our story'), faq: 'FAQ', custom: t('Pesanan khusus', 'Custom orders') };
  ...
  ({ home, shop, product, cart: cartPage, checkout, about, faq, custom }[page] || home)();
